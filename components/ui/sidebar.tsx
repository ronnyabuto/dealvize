...
>(
  (
    ...
    const open = isMobile ? (openProp ?? _open) : true // Always true on desktop
    ...
    const setOpen = React.useCallback(
      (value: boolean | ((value: boolean) => boolean)) => {
        ...
        // On desktop, ignore close requests - always stay open
      },
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-7 w-7", !isMobile && "opacity-50 cursor-not-allowed", className)}
      onClick={(event) => {
        onClick?.(event)
        // Only allow toggle on mobile
        if (isMobile) {
